...
int calculate2::gcd(int x,int y)
{
	int r=1,t;
	if(x>y){t=x;x=y;y=t;}
	while(r!=0)
...
int calculate2::B1(int d[],int c[],int fenshu[])
{
	int j;
	cout<<'('<<d[0]<<char(c[0])<<d[1]<<')'<<char(c[1])<<d[2]<<char(c[2])<<d[3]<<'='<<endl;
	for(j=0;j<3;j++)
...
int calculate2::B3(int d[],int c[],int fenshu[])
{
	int j;
	cout<<d[0]<<char(c[0])<<'('<<d[1]<<char(c[1])<<d[2]<<')'<<char(c[2])<<d[3]<<'='<<endl;
	for(j=0;j<3;j++)
...
int calculate2::C1(int d[],int c[],int fenshu[])
{
	int j;
	cout<<d[0]<<char(c[0])<<'('<<d[1]<<char(c[1])<<d[2]<<char(c[2])<<d[3]<<')'<<'='<<endl;
	for(j=0;j<3;j++)
